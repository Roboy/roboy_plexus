...
#
#  Copyright (c) 2004-2020 Canonical Ltd.
...
class UpdateSubscriptions(object):
    ...
    def get_status(self):
        if self.other:
            return UPDATES_CUSTOM
        if self.security and self.updates and self.backports:
            return UPDATES_ALL
        elif self.security and self.updates and not self.backports:
            return UPDATES_RECOMMENDED
        elif self.security and not self.updates and not self.backports:
            return UPDATES_SECURITY
        return UPDATES_CUSTOM


def maybe_log_authentication_canceled_error(dbus_exception):
    """Log an error if the DBus exception was raised because the user canceled
...
class SoftwarePropertiesGtk(SoftwareProperties, SimpleGtkbuilderApp):
    ...
    def show_distro(self):
        ...
        for template in self.distro.source_template.children:
            ...
            subscriptions.inconsistent |= inconsistent
            if active:
                if pocket == POCKET_SECURITY:
                    subscriptions.security = True
                elif pocket == POCKET_UPDATES:
                    subscriptions.updates = True
                elif pocket == POCKET_BACKPORTS:
                    subscriptions.backports = True
                else:
                    subscriptions.other = True

        status = subscriptions.get_status()
        combobox = self.combobox_updates_subscription
        model = combobox.get_model()
        if status == UPDATES_CUSTOM:
            try:
            ...
            try:
                ...
                pass
        combobox.set_active(status)
        combobox.set_sensitive(not subscriptions.inconsistent)
    ...
    def on_combobox_updates_subscription_changed(self, combobox):
        ...
        def _toggle_child_source(pocket, enabled):
        ...
        try:
            if index == UPDATES_ALL:
                _toggle_child_source(POCKET_SECURITY, True)
                _toggle_child_source(POCKET_UPDATES, True)
                _toggle_child_source(POCKET_BACKPORTS, True)
            elif index == UPDATES_RECOMMENDED:
                _toggle_child_source(POCKET_SECURITY, True)
                _toggle_child_source(POCKET_UPDATES, True)
                _toggle_child_source(POCKET_BACKPORTS, False)
            elif index == UPDATES_SECURITY:
                _toggle_child_source(POCKET_SECURITY, True)
                _toggle_child_source(POCKET_UPDATES, False)
                _toggle_child_source(POCKET_BACKPORTS, False)
            for template in self.distro.source_template.children:
                if template.type == "deb-src":
                    continue
                ...
                if pocket not in (POCKET_SECURITY, POCKET_UPDATES, POCKET_BACKPORTS):
                    _toggle_child_source(pocket, False)
        except dbus.DBusException as e:
    ...
    def remove_key_clicked(self, widget):
        ...
        try:
            if not self.backend.RemoveKey(key[:16]):
                error(self.main,
