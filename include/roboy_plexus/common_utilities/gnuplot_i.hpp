...
public:
	...
	// -----------------------------------------------
	inline Gnuplot& set_multiplot(int* layout, float* scale){
		char str[100];
		sprintf(str,"set multiplot layout %d,%d rowsfirst scale %.1f,%.1f",layout[0],layout[1],scale[0],scale[1]);
		cmd(str) ;
		multiplot=true;
		return *this;};

	inline Gnuplot& set_multiplot(int* layout){
		char str[100];
		sprintf(str,"set multiplot layout %d,%d rowsfirst",layout[0],layout[1]);
		cmd(str) ;
		multiplot=true;
		return *this;};
	...
	/// \return <-- reference to the gnuplot object
	// -----------------------------------------------
	inline Gnuplot& set_origin(float x, float y){
		char str[50];
		sprintf(str,"set origin %.1f,%.1f",x,y);
		cmd(str) ;
		return *this;};
	...
	/// \return <-- reference to the gnuplot object
	// -----------------------------------------------
	inline Gnuplot& unset_multiplot(){cmd("unset multiplot");multiplot=false;return *this;};
							 ...
							 const std::string &title)
{
	...
	// command to be sent to gnuplot
	//
	if (nplots > 0  &&  two_dim == true && multiplot==false)
		cmdstr << "replot ";
	else
		cmdstr << "plot ";
							   ...
							   const std::string &title)
{
	...
	// command to be sent to gnuplot
	//
	if (nplots > 0  &&  two_dim == false && multiplot==false)
		cmdstr << "replot ";
	else
		cmdstr << "splot ";
