...
*/
/**************************************************************************/
Adafruit_BNO055::Adafruit_BNO055(int32_t sensorID, uint8_t address, void * h2p_lw_i2c_addr)
{
    _sensorID = sensorID;
    _address = address;
    i2c = new I2C(h2p_lw_i2c_addr);
...
bool Adafruit_BNO055::getEvent(sensors_event_t *event)
{
    ...
    event->timestamp = (int)time_span.count();
    imu::Vector<3> vec;
    switch(type){
...
{
    i2c->write(_address, (reg<<24|value<<16), 2);
...
{
    vector<uint8_t> data;
    i2c->read(_address, reg, len, data);
